...
Redeven Desktop keeps one shared launcher surface across five entry families: local environments, provider environments, provider local serves, saved Redeven Local UI URLs, and saved SSH targets. Connecting a Control Plane refreshes the provider catalog in place, but Desktop no longer materializes remote-only provider environments into `managed_environments`. Remote provider cards stay remote-first, while any on-device runtime for that provider environment becomes its own separate `Local Serve` card.

The launcher Environment Library keeps those entries in one consistent card system: pinned environments surface in a dedicated `Pinned` section first, every card uses the same compact facts layout, Endpoint values render as readonly copy-friendly inputs, and provider-backed environments can be narrowed through a lightweight provider filter. Managed cards also surface local-runtime ownership so the primary action can read as `Open`, `Attach`, or `Focus` instead of hiding whether Desktop will start a runtime, reuse one, or just refocus a window. The `Control Planes` tab is now provider management only: it shows account state, sync health, published environment counts, local-host counts, and a shortcut back into the filtered Environment list.

Desktop can open any desktop-managed local environment, open a provider environment remotely through a Control Plane, save a provider-backed local serve on this device, attach to a reachable Redeven Local UI URL, or SSH into another machine. Local environment creation is always local-only. When a provider environment also needs an on-device runtime, Desktop creates a separate provider-local-serve record instead of merging local and remote access into one card.
