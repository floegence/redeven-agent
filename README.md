...
Redeven Desktop keeps one shared launcher surface across four persisted entry families: local environments, provider environments, saved Redeven Local UI URLs, and saved SSH Host entries. Connecting a Control Plane refreshes the provider catalog in place and reconciles it into first-class `provider_environments` records. A provider-backed environment always stays one card: remote access through the Control Plane and any optional on-device local runtime both live on that same card.

The launcher Environment Library keeps those entries in one consistent card system: pinned environments surface in a dedicated `Pinned` section first, every card uses the same compact facts layout, Endpoint values render as readonly copy-friendly inputs, and provider-backed environments can be narrowed through a lightweight provider filter. Runtime health and window state are separated explicitly: every card shows `RUNTIME ONLINE` or `RUNTIME OFFLINE`, the primary action is only `Open` / `Opening…` / `Focus`, and runtime start/stop lives in the adjacent runtime menu. The `Control Planes` tab is now provider management only: it shows account state, sync health, published environment counts, local-host counts, and a shortcut back into the filtered Environment list.

Desktop can open any desktop-managed local environment, open a provider environment remotely through a Control Plane, attach a local runtime to that same provider environment when needed, attach to a reachable Redeven Local UI URL, or deploy/open an SSH Host environment on another machine. Local environment creation is always local-only. Provider cards expose explicit route actions such as `Open via Control Plane` and `Set up local runtime…`, so debug and validation flows stay predictable without splitting one provider environment into multiple cards. Closing a window no longer implies stopping its runtime: local environments, provider environments with a configured local runtime, and SSH Host entries stop only through the explicit runtime menu, while remote-only provider and direct URL entries remain observe-only.

For SSH Host environments, Desktop now supports three bootstrap delivery modes:
...
Desktop only reuses a remote runtime when the binary reports the exact current Desktop release and the version root contains a valid Desktop-managed runtime stamp. Shared release artifacts live under `releases/<release_tag>/`, while mutable remote state lives under `instances/<environment_instance_id>/`, so multiple people or devices can use the same SSH-reachable host without silently colliding. Unrelated user-managed `redeven` installs elsewhere on the host are left alone and are not adopted automatically.
