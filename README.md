...
- After mirror integrity verification succeeds, the workflow deploys the version-manifest Worker for `https://version.agent.redeven.com/v1/manifest.json` (no manifest bucket required).
- `install.sh` downloads from GitHub first, then falls back to Cloudflare mirror.
- Installer worker deployment (`redeven.com/install.sh`) stays on Cloudflare Workers Builds and is triggered only via the `release` branch flow.
