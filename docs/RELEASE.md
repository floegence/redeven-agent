...
- URL: `https://version.agent.redeven.com/v1/manifest.json`
...
- Fallback: Cloudflare package mirror (`agent.package.redeven.com`)

## Install worker deployment (separate from package mirror)

Cloudflare Worker deployment for `redeven.com/install.sh` is managed by **Cloudflare Workers Builds** (GitHub integration), not by GitHub Actions.
