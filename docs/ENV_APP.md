...
- Terminal selection copy stays explicit through shared terminal infrastructure: `copyOnSelect` remains disabled, `floeterm` owns `Cmd/Ctrl+C` selection-copy arbitration when the hidden terminal input is focused, the shortcut falls through unchanged when no terminal selection exists, and Env App's `Copy selection` context-menu action now delegates to the same upstream `copySelection('command')` path instead of maintaining a product-local clipboard fork.
