...
- Pointer/context-menu ownership and wheel ownership are separate contracts. Ordinary `data-floe-canvas-interactive="true"` widget regions stay eligible for cursor-centered canvas zoom, `data-floe-local-interaction-surface="true"` descendants still keep local pointer/context-menu ownership, and explicit `data-floe-canvas-wheel-interactive="true"` business surfaces keep wheel local only when they belong to the currently selected widget.
