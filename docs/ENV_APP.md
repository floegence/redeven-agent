...
- Terminal tabs now use shell-integration status chrome instead of a textual attention prefix: background commands show a compact spinner while they are running, completed/background activity or bell events collapse into an unread dot without a bell toast, and reopening the tab clears the unread marker.
