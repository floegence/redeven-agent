...
- Env App owns a root-scoped `--redeven-surface-panel*` theme family for the standard light/dark panel pairing; shared `card` / `popover` surfaces and Flower chat consume that same contract so body portals and in-tree panels stay visually aligned.
