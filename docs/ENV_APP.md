...
- Codex transcript evidence also stays surface-local: file-change rows render through a Codex-owned git-patch style block, and reasoning / plan rows start collapsed while preserving per-item disclosure state across streaming updates.
...
- Flower transcript clearance for that floating composer is measured from the real bottom-dock height and written back into the transcript inset contract, so autosizing textareas, attachment strips, queued follow-up panels, and mobile safe-area changes cannot hide the newest visible transcript output under the composer.
