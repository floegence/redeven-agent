...
- Flower assistant live rendering treats reasoning as visible content, not hidden telemetry. Realtime transport may coalesce frequent low-priority assistant updates, but the live UI must converge to the same complete reasoning/markdown content already preserved in the active-run snapshot and persisted transcript.
