...
- Clean Git workspace states also keep the header intentionally quiet: irrelevant disabled actions are removed instead of lingering as inactive chrome, while directory breadcrumbs reuse the collapsed middle-segment pattern so narrow panes preserve the root and current scope without wrapping into multiple lines.
