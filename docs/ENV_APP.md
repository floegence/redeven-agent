...
- Flower assistant live rendering now keeps persisted transcript and active run state as separate layers. The virtualized transcript renders settled messages only; the in-flight assistant run renders in one dedicated live-run footer inside the same scroll container.
- Flower assistant live output keeps `thinking` hidden from the default transcript view. Live answer text streams through an append-only plain-text path, while markdown parsing is a settled-transcript concern only after the canonical assistant message lands in persisted transcript history.
- Active-run snapshots and realtime assistant events reconcile into one live assistant state source. The transcript projection no longer fabricates assistant overlay rows, and follow-bottom behavior reacts to footer growth instead of repeated transcript row remeasurement.
- While the live footer is active, render identity must stay stable by block position. Live answer text nodes must append in place rather than remount, and the inline cursor must stay attached to the tail of the latest visible live answer block.
- Transcript-only affordances such as message timestamps and copy actions stay attached to settled transcript messages; the live footer remains a lean progress surface and disappears once transcript catches up or the run ends without visible assistant output.
