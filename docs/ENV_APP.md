...
- The Codex sidebar/page use floe-webapp layout and form primitives for visual consistency, but Codex state, icon assets, thread navigation, and request handling stay implemented as a separate surface rather than as Flower extensions.
- The Codex surface is structured as a dedicated review navigator plus artifact-review workbench, so host diagnostics, approvals, transcript evidence, and artifact previews stay together without reusing Flower chat IA.
