...
- Workbench layout now follows the same runtime-authoritative pattern for cross-terminal consistency: Env App fetches `/_redeven_proxy/api/workbench/layout/snapshot`, subscribes to `/_redeven_proxy/api/workbench/layout/events`, and persists shared layout replacements through `PUT /_redeven_proxy/api/workbench/layout`, while the runtime stores that shared scene in `<state_dir>/apps/workbench/layout.sqlite`.
...
- Runtime-shared workbench state is intentionally narrow: only widget identity/type plus geometry (`x`, `y`, `width`, `height`, `z_index`, `created_at_unix_ms`) are shared across terminals; viewport pan/zoom, current selection, transient drag/resize state, dynamic titles, terminal tab groups, file-browser location, preview targets, and other widget-local business context stay local to each client.
- Runtime workbench synchronization is coarse-grained by design: the runtime publishes ordered `layout.replaced` snapshot events instead of fine-grained widget patches, so every client can deterministically reproject the same shared widget scene without coupling transient browser UI state to the runtime store.
- Remote workbench updates must not move the local camera. Env App reapplies the shared widget scene while preserving each terminal's own viewport and local widget state, so cross-terminal consistency covers opened components and their positions without stealing the current viewing or editing context.
...
- Pointer/context-menu ownership and wheel ownership are separate contracts. Ordinary `data-floe-canvas-interactive="true"` widget regions stay eligible for cursor-centered canvas zoom, while `data-floe-local-interaction-surface="true"` descendants still keep local pointer/context-menu ownership and explicit `data-floe-canvas-wheel-interactive="true"` business surfaces keep wheel local for true scroll consumers such as file-browser panes, terminal surfaces, monitoring pages, and inline conversation shells.
...
- Workbench multi-instance state is product-owned. Generic floe-webapp workbench state still owns widget geometry, selection, and filters, while Redeven keeps a separate instance-state layer for `latestWidgetIdByType` and per-terminal widget session-group membership so multi-instance Terminal and Files routing stays explicit without turning every workbench widget into a multi-instance surface.
- Workbench Terminal windows are panel-group scoped rather than mirrored views of one global session strip. Runtime terminal sessions still live in the shared coordinator, but each workbench Terminal widget owns its own visible session-id group and active tab, so opening a new Terminal window creates a separate tab set instead of another copy of the same tabs.
- Workbench Files windows use stable widget-scoped persistence keys rather than Deck widget state, so each Files widget restores its own path, Git subview, and hidden-file preference independently across reloads.
