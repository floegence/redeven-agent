...
- Git browse scroll ownership is explicit and shared. Redeven marks real Git scroll viewports through one `GIT_WORKBENCH_SCROLL_REGION_PROPS` helper, so selected Git widgets keep wheel scrolling local across overview/status/history/compare/stash panes while unselected widgets still leave wheel zoom on the canvas.
