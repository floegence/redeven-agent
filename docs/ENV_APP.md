...
- Terminal selection copy stays explicit and product-owned: `copyOnSelect` remains disabled, `Cmd/Ctrl+C` copies only when the active terminal currently has selected text, the shortcut falls through unchanged when no terminal selection exists, and the custom `Copy selection` context-menu action uses that same clipboard path instead of a separate behavior fork.
