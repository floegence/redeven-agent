...
- Notes overlay keyboard ownership is split deliberately: floe-webapp floating Notes now preserves only an explicit allowlist of shell shortcuts, and Env App forwards its single shell-owned Notes toggle keybind (`mod+.`) into that allowlist so the overlay can still close after canvas or note interactions without reopening the rest of the shell hotkey surface.
