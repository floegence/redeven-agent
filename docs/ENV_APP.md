...
- Flower chat composer now stays Flower-owned instead of keeping keyboard/storage behavior inline in `EnvAIPage.tsx`: env-scoped prompt history preserves a reversible local draft snapshot (including attachments), and first-line slash commands (`/clear`, `/plan`, `/act`, `/cwd`) are handled inside the composer without coupling Flower to Codex-only UI modules.
