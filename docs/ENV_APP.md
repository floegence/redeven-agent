...
- Git browser pills now follow the same semantic contract as the dots instead of page-level utility pairs: Git widgets stamp semantic pill metadata in the DOM, `src/styles/redeven.css` resolves the final light/dark token family, light theme pills intentionally stay dark with light text, dark theme pills invert to light fills with dark borders/text, and selected chips keep that inversion while still attaching to the active Git selection surface.
