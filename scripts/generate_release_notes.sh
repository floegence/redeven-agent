...
\`\`\`bash
curl -fsSL https://redeven.com/install.sh | REDEVEN_VERSION=${release_tag} sh
\`\`\`

Upgrade an existing agent in place:

\`\`\`bash
curl -fsSL https://redeven.com/install.sh | REDEVEN_INSTALL_MODE=upgrade REDEVEN_VERSION=${release_tag} sh
\`\`\`
...
- Version manifest endpoint: https://version.agent.redeven.com/v1/manifest.json
