              ...
              return (
                <>
                  <div class="flex-1 min-h-0 overflow-auto px-3 py-3">
                    <div class="space-y-2">
                      <section class={cn('rounded-xl border p-2.5', gitToneSurfaceClass(branchTone()))}>
                        <div class="flex flex-wrap items-start justify-between gap-2.5">
                          <div class="min-w-0 flex-1">
                            ...
                            <Show when={branch.subject}>
                              <div class={cn('mt-2.5 rounded-lg border px-2.5 py-2 text-[11px] leading-5 text-foreground', gitToneInsetClass(branchTone()))}>{branch.subject}</div>
                            </Show>
                          </div>
                      ...
                      <div class="grid grid-cols-1 gap-2 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
                        ...
                        <section class={cn('rounded-xl border p-2.5', gitToneSurfaceClass('neutral'))}>
                          <div class="text-[10px] font-medium uppercase tracking-[0.16em] text-muted-foreground/70">Branch State</div>
                          <div class="mt-2.5 grid grid-cols-1 gap-2 text-[11px] sm:grid-cols-2">
                            <div class={cn('rounded-lg border px-2.5 py-1.5', gitToneInsetClass('neutral'))}>
                            ...
                            <div class={cn('rounded-lg border px-2.5 py-1.5', gitToneInsetClass(branch.worktreePath ? 'info' : 'neutral'))}>
                        ...
                        <Show when={!props.compareLoading} fallback={<div class="mt-3 text-xs text-muted-foreground">Loading compare files...</div>}>
                          <Show when={!props.compareError} fallback={<div class="mt-3 text-xs break-words text-error">{props.compareError}</div>}>
                            <Show when={(props.compare?.files.length ?? 0) > 0} fallback={<div class="mt-3 text-xs text-muted-foreground">No changed files in compare.</div>}>
                              <div class="mt-2.5 grid grid-cols-1 gap-1.5 xl:grid-cols-2">
