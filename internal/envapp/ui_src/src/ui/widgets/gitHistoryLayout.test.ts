...
describe('browser workspace layout wiring', () => {
  ...
  it('routes branch review through status and history views with compare in a dialog', () => {
    ...
    expect(branchesSrc).toContain('flex w-full min-w-0 flex-col gap-2 lg:flex-[0_1_20rem] lg:max-w-[min(50%,22rem)] lg:items-stretch');
    expect(branchesSrc).toContain("const headerActionDeckClass = 'space-y-1.5 rounded-lg border border-border/60 bg-muted/[0.14] p-1.5 shadow-sm shadow-black/5';");
    expect(branchesSrc).toContain("const headerActionDeckLabelClass = 'px-1 text-[10px] font-medium uppercase tracking-[0.16em] text-muted-foreground/60';");
    expect(branchesSrc).toContain("const headerActionButtonClass = 'w-full rounded-md bg-background/88 shadow-sm shadow-black/5';");
    expect(branchesSrc).toContain('>Workspace<');
    expect(branchesSrc).toContain('>Actions<');
    expect(branchesSrc).toContain('>View<');
    expect(branchesSrc).toContain("class={cn('grid gap-1.5', workspaceHelperGridClass())}");
    expect(branchesSrc).toContain("class={cn('grid gap-1.5', branchActionGridClass())}");
    expect(branchesSrc).toContain('grid w-full grid-cols-2 rounded-md border border-border/65 bg-background/72 p-0.5');
