...
export function GitViewNav(props: GitViewNavProps) {
  ...
  const badgeBaseClass =
    'inline-flex min-w-[1.75rem] items-center justify-center rounded-md px-1.5 py-0.5 text-[10px] font-medium tabular-nums transition-colors duration-150';

  return (
    <div class={cn('space-y-1 rounded-lg bg-muted/[0.14] p-1', props.class)} role="tablist" aria-label="Git views">
          ...
          return (
            <button
              ...
              class={cn(
                buttonBaseClass,
                active()
                  ? 'border-l-[2px] border-primary bg-background text-foreground shadow-sm'
                  : 'bg-transparent text-muted-foreground hover:bg-muted/50 hover:text-foreground hover:shadow-[0_1px_2px_rgba(0,0,0,0.04)]',
              )}
