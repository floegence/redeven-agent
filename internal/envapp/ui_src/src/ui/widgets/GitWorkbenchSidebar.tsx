...
export function GitWorkbenchSidebar(props: GitWorkbenchSidebarProps) {
  ...
  return (
    <SidebarPane
      title="Git"
      headerActions={<span class="text-[10px] uppercase tracking-[0.18em] text-muted-foreground/70">Navigator</span>}
    ...
    >
      <div class="flex h-full min-h-0 flex-col">
        <div class="shrink-0 border-b border-border/70 bg-background/95 px-2.5 py-3 backdrop-blur supports-[backdrop-filter]:bg-background/85">
          <div class="space-y-3">
            ...
            <div class="space-y-1.5">
              <div class="flex items-center justify-between px-1 text-[10px] font-medium uppercase tracking-[0.16em] text-muted-foreground/70">
                <span>View</span>
                <span>{subviewTitle(props.subview)}</span>
              </div>
              <GitSubviewSwitch
                value={props.subview}
                items={subviewItems()}
                onChange={props.onSubviewChange}
                class="w-full"
              />
            </div>
          </div>
            ...
            <Show when={!props.repoInfoError} fallback={<div class="py-3 text-xs break-words text-error">{props.repoInfoError}</div>}>
              <Show when={props.repoAvailable} fallback={<div class="py-3 text-xs text-muted-foreground">Current path is not inside a Git repository.</div>}>
                <div class="flex h-full min-h-0 flex-col overflow-hidden">
                  <div class="flex items-center justify-between px-1 pb-2 text-[10px] font-medium uppercase tracking-[0.16em] text-muted-foreground/70">
                    <span>{props.subview === 'overview' ? 'Focus' : 'Selection'}</span>
                    <span>{subviewTitle(props.subview)}</span>
                  </div>

                  <div class="min-h-0 flex-1 overflow-hidden">
                    <Show when={props.subview === 'overview'}>
                      <div class="h-full min-h-0 space-y-3 overflow-auto pr-1">
                        <div class="rounded-lg border border-border/70 bg-muted/15 p-3">
                          ...
                          <div class="mt-1 text-[11px] text-muted-foreground">{workspaceCount() > 0 ? 'Files need review in the Changes view.' : 'Working tree is clean.'}</div>
                        </div>
                        ...
                        <div class="rounded-lg border border-dashed border-border/70 bg-background/70 p-3 text-[11px] leading-5 text-muted-foreground">
                          Use the view switch above to move from overview into changes, branches, or commit details without duplicating controls in the content area.
                        </div>
                      </div>
                    </Show>
