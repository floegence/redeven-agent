...
export const CodexMessageRunIndicator: Component<CodexMessageRunIndicatorProps> = (props) => {
  const filterID = `codex-message-run-indicator-${createUniqueId()}`;
  ...
  return (
    <div class={cn('codex-message-run-indicator', props.class)} role="status" aria-live="polite">
      <div class="codex-message-run-indicator-surface">
        <svg class="codex-message-run-indicator-graph" viewBox="0 0 40 40" fill="none" aria-hidden="true">
          <defs>
            <filter id={filterID}>
            ...
            <For each={TO_CENTER_CONNECTIONS}>
              {([from, to], index) => (
                <line
                  ...
                  stroke-dasharray="1"
                  stroke-dashoffset="1"
                  class="codex-message-run-indicator-center-line"
            ...
            <For each={SIDE_CONNECTIONS}>
              {([from, to], index) => (
                <line
                  ...
                  x2={RUN_INDICATOR_NODES[to].x}
                  y2={RUN_INDICATOR_NODES[to].y}
                  class="codex-message-run-indicator-side-line"
            ...
            <For each={RUN_INDICATOR_NODES}>
              {(node, index) => (
                <circle
                  ...
                  r={index() === 5 ? 2.5 : 2}
                  fill="var(--primary)"
                  class="codex-message-run-indicator-node"
        ...
        <div class="codex-message-run-indicator-bars" aria-hidden="true">
          ...
          <span class="codex-message-run-indicator-bar" style={{ 'animation-delay': '100ms' }} />
          <span class="codex-message-run-indicator-bar" style={{ 'animation-delay': '200ms' }} />
          <span class="codex-message-run-indicator-bar" style={{ 'animation-delay': '300ms' }} />
        </div>
      </div>
    </div>
  );
