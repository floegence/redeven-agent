      ...
      >
        <span
          class={`w-1.5 h-1.5 rounded-full ${
            props.running ? 'bg-emerald-500 animate-pulse' : 'bg-zinc-400'
          }`}
        />
      ...
      <CardFooter class="pt-2 flex items-center justify-between gap-2 border-t border-border/50">
        ...
        <div class="flex items-center gap-1">
          ...
          <Tooltip content="Delete codespace" placement="top">
            <Button size="sm" variant="ghost" disabled={props.busy} onClick={props.onDelete} class="px-2 text-muted-foreground hover:text-destructive">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
export function EnvCodespacesPage() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await fetchGatewayJSON<GatewayResp<void>>(`/_redeven_proxy/api/spaces/${encodeURIComponent(target.code_space_id)}`, { method: 'DELETE' });
      await refetch();
