...
export function EnvAIPage() {
  ...
  const resetContextTelemetryState = () => {
    ...
    activeContextReplaySeqByRun.clear();
  };
  ...
  const loadContextRunEvents = async (
    ...
    if (!rid) return;
    if (!contextTelemetryController.ensureRun(rid)) return;

    const threadIdAtStart = String(ai.activeThreadId() ?? '').trim();
    const resetSeqAtStart = activeContextResetSeq;
    const reqSeq = (activeContextReplaySeqByRun.get(rid) ?? 0) + 1;
    activeContextReplaySeqByRun.set(rid, reqSeq);
    const currentRunState = untrack(contextTelemetryByRun)[rid] ?? null;
    let cursor = opts?.reset ? 0 : Math.max(0, Number(currentRunState?.cursor ?? 0) || 0);
    ...
    try {
      while (pages < maxPages) {
        ...
        if (resetSeqAtStart !== activeContextResetSeq) return;
        if (reqSeq !== activeContextReplaySeqByRun.get(rid)) return;
        if (threadIdAtStart !== String(untrack(ai.activeThreadId) ?? '').trim()) return;
      ...
      if (
        resetSeqAtStart === activeContextResetSeq
        && reqSeq === activeContextReplaySeqByRun.get(rid)
        && threadIdAtStart === String(untrack(ai.activeThreadId) ?? '').trim()
      ) {
  ...
  createEffect(() => {
    ...
    const unsub = ai.onRealtimeEvent((event) => {
      ...
      if (event.eventType === 'stream_event') {
        ...
        const isActiveTid = tid === String(ai.activeThreadId() ?? '').trim();
        const isContextStreamEvent = isActiveTid && (streamKind === 'context' || streamType === 'context-usage' || streamType === 'context-compaction');
        if (isContextStreamEvent && eventRunID) {
          contextTelemetryController.setLiveRun(eventRunID);
          contextTelemetryController.setStableRun(eventRunID);
        }
        if (isContextStreamEvent) {
  ...
  createEffect(() => {
    ...
    const liveRunId = String(ai.runIdForThread(tid) ?? '').trim();
    const binding = contextTelemetryController.setLiveRun(liveRunId);
    if (!liveRunId) return;
    void loadContextRunEvents(liveRunId, {
  ...
  createEffect(() => {
    ...
    if (!tid) return;
    const liveRunId = String(ai.runIdForThread(tid) ?? '').trim();
    const stableRunId = String(ai.lastContextRunIdForThread(tid) ?? '').trim();
    const binding = contextTelemetryController.setStableRun(stableRunId);
    if (!stableRunId || stableRunId === liveRunId) return;
    void loadContextRunEvents(stableRunId, {
      reset: binding.switched,
      maxPages: RUN_CONTEXT_EVENTS_MAX_PAGES,
    });
  });
  ...
  createEffect(() => {
    ...
    const runId = String(contextTelemetryController.liveContextRunId() ?? ai.runIdForThread(tid) ?? '').trim();
    if (!runId) return;
  ...
  const sendUserTurn = async (content: string, attachments: Attachment[], opts: SendUserTurnOptions = {}) => {
    ...
    try {
      ...
      if (rid) {
        ...
        void loadActiveRunSnapshot(tid);
        const binding = contextTelemetryController.setLiveRun(rid);
        void loadContextRunEvents(rid, { reset: binding.switched });
              ...
              <div class="flower-chat-shell">
                ...
                <div class="flower-chat-main">
                  ...
                  >
                    ...
                    <div class="flower-chat-bottom-dock-lane flower-chat-bottom-dock-lane-composer">
                      <div class="flower-chat-bottom-dock-track">
                        <AIChatInput
                          class="flower-chat-input flower-chat-input-floating"
                          disabled={!canInteract()}
