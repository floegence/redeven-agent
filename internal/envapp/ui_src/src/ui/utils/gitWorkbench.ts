...
type GitDiffEntryLike =
  | GitWorkspaceChange
  | GitCommitFileSummary
  | GitDiffFileContent;

export type GitWorkbenchSubview =
  | "overview"
  | "changes"
  | "branches"
  | "history";
...
export type GitStashWindowSource =
  | "header"
  | "changes"
  | "branch_status"
  | "merge_blocker";
...
export type GitSeededDiffEntry<
  T extends GitCommitFileSummary | GitWorkspaceChange,
> = T & GitSeededDiffFields;
export type GitSeededCommitFileSummary =
  GitSeededDiffEntry<GitCommitFileSummary>;
export type GitSeededWorkspaceChange = GitSeededDiffEntry<GitWorkspaceChange>;
export type GitSeededWorkspaceChangesResponse = Omit<
  GitListWorkspaceChangesResponse,
...
export function isGitMergeCommitPresentation(
  presentation: GitCommitDiffPresentation | null | undefined,
): boolean {
  return (
    Boolean(presentation?.mergeCommit) ||
    Number(presentation?.parentCount ?? 0) > 1 ||
    String(presentation?.mode ?? "").trim() === "first_parent"
  );
}

export function isGitFirstParentCommitPresentation(
  presentation: GitCommitDiffPresentation | null | undefined,
): boolean {
  return String(presentation?.mode ?? "").trim() === "first_parent";
}

export function gitCommitDiffPresentationBadge(
  presentation: GitCommitDiffPresentation | null | undefined,
): string {
  if (!isGitMergeCommitPresentation(presentation)) return "";
  return "Merge Commit";
}

export function gitCommitDiffPresentationDetail(
  presentation: GitCommitDiffPresentation | null | undefined,
): string {
  if (isGitFirstParentCommitPresentation(presentation)) {
    return "Compared with first parent so the changed-file list and diff view stay aligned.";
  }
  if (isGitMergeCommitPresentation(presentation)) {
    return "Showing merge commit diff context.";
  }
  return "";
}
...
export const WORKSPACE_REVIEW_SECTIONS: GitWorkspaceSection[] = [
  "unstaged",
  "untracked",
  "conflicted",
  "staged",
];
export const WORKSPACE_VIEW_SECTIONS: GitWorkspaceViewSection[] = [
  "changes",
  "conflicted",
  "staged",
];
...
export function createEmptyWorkspaceViewPageStateRecord(): Record<
  GitWorkspaceViewSection,
  GitWorkspaceViewPageState
> {
  return {
...
export function summarizeWorkspaceCount(
  summary: GitWorkspaceSummary | null | undefined,
): number {
  return (
    Number(summary?.stagedCount ?? 0) +
...
export function summarizePendingWorkspaceCount(
  summary: GitWorkspaceSummary | null | undefined,
): number {
  return (
    Number(summary?.unstagedCount ?? 0) +
...
export function buildGitWorkbenchSubviewItems(params: {
  ...
  const summary =
    params.workspace?.summary ?? params.repoSummary?.workspaceSummary;
  return [
    {
      id: "changes",
      label: "Changes",
...
export function describeGitHead(
  primary?: Pick<
    GitRepoSummaryResponse,
    "headRef" | "headCommit" | "detached"
  > | null,
  fallback?:
    | Pick<GitRepoSummaryResponse, "headRef" | "headCommit" | "detached">
    | Pick<{ headRef?: string; headCommit?: string }, "headRef" | "headCommit">
    | null,
): GitHeadDisplay {
  const headRef = String(primary?.headRef ?? fallback?.headRef ?? "").trim();
  const headCommit = String(
    primary?.headCommit ?? fallback?.headCommit ?? "",
  ).trim();
  const detached =
    Boolean(primary?.detached) ||
    Boolean(
      (fallback as { detached?: boolean } | null | undefined)?.detached,
    ) ||
    headRef === "" ||
    headRef === "HEAD" ||
    headRef === "(detached)";
...
export function reattachBranchFromRepoSummary(
  summary?: GitRepoSummaryResponse | null,
...
export function detachedHeadViewingSummary(
  headCommit: string | null | undefined,
): string {
  const detail = shortGitHash(headCommit);
  return detail
    ? `Viewing ${detail} without a branch.`
    : "Viewing repository history without a branch.";
}
...
export function workspaceViewSectionLabel(
...
export function workspaceViewBulkActionLabel(
  section: GitWorkspaceViewSection,
): string {
...
export function workspaceSectionActionKey(
...
export function workspaceViewSectionCount(
  summary: GitWorkspaceSummary | null | undefined,
  section: GitWorkspaceViewSection,
): number {
  if (section === "changes") {
    return (
      Number(summary?.unstagedCount ?? 0) + Number(summary?.untrackedCount ?? 0)
    );
  }
...
export function applyWorkspaceViewPageSnapshot(
  ...
  const base: GitListWorkspaceChangesResponse =
    workspace && workspace.repoRootPath === page.repoRootPath
      ? workspace
      : {
  ...
  switch (page.section) {
    ...
    case "conflicted": {
      next.conflicted = append
        ? [...next.conflicted, ...page.items]
        : [...page.items];
      break;
    }
    case "changes":
    default: {
      const unstagedItems = page.items.filter(
        (item) => item.section === "unstaged",
      );
      const untrackedItems = page.items.filter(
        (item) => item.section === "untracked",
      );
      next.unstaged = append
        ? [...next.unstaged, ...unstagedItems]
        : unstagedItems;
      next.untracked = append
        ? [...next.untracked, ...untrackedItems]
        : untrackedItems;
      break;
    }
  }
...
export function gitDiffEntryIdentity(
...
export function pickDefaultWorkspaceSection(
  workspace: GitListWorkspaceChangesResponse | null | undefined,
): GitWorkspaceSection {
...
export function pickDefaultWorkspaceViewSection(
  ...
  for (const section of WORKSPACE_VIEW_SECTIONS) {
    if (workspaceViewSectionItems(workspace, section).length > 0)
      return section;
  }
  return "changes";
}
...
export function findWorkspaceChangeByKey(
  ...
  for (const section of WORKSPACE_SECTIONS) {
    const item = workspaceSectionItems(workspace, section).find(
      (entry) => workspaceEntryKey(entry) === wanted,
    );
    if (item) return item;
  }
  return null;
...
export function allGitBranches(
...
export function findGitBranchByKey(
  ...
  if (!wanted) return null;
  return (
    allGitBranches(branches).find(
      (branch) => branchIdentity(branch) === wanted,
    ) ?? null
  );
}

export function pickDefaultGitBranch(
  branches: GitListBranchesResponse | null | undefined,
): GitBranchSummary | null {
...
export function branchContextSummary(
  branch: GitBranchSummary | null | undefined,
): string {
...
export function branchStatusSummary(
  branch: GitBranchSummary | null | undefined,
): string {
  if (!branch) return "No branch selected";
  const parts: string[] = [];
...
export function compareHeadline(
  compare: GitGetBranchCompareResponse | null | undefined,
): string {
  ...
  if (ahead <= 0 && behind <= 0)
    return "Compared branch matches the reference branch.";
  if (ahead > 0 && behind <= 0)
    return `Compared branch is ahead by ${ahead} commit${ahead === 1 ? "" : "s"}.`;
  if (behind > 0 && ahead <= 0)
    return `Compared branch is behind by ${behind} commit${behind === 1 ? "" : "s"}.`;
...
export function workspaceHealthLabel(
  ...
  if (staged > 0 && pending > 0)
    return `${staged} staged, ${pending} pending review.`;
...
export function changeMetricsText(
  change: GitDiffEntryLike | null | undefined,
): string {
  return `+${change?.additions ?? 0} / −${change?.deletions ?? 0}`;
}

export function workspaceMutationPaths(
...
export function isGitWorkspaceSection(
  value: unknown,
): value is GitWorkspaceSection {
  return WORKSPACE_SECTIONS.includes(value as GitWorkspaceSection);
}

export function recountWorkspaceSummary(
  workspace: GitListWorkspaceChangesResponse | null | undefined,
): GitWorkspaceSummary {
  return {
...
export function applyWorkspaceSectionMutation(
  ...
  params: {
    ...
    destinationSection:
      | GitWorkspaceSection
      | ((change: GitWorkspaceChange) => GitWorkspaceSection);
  },
): GitListWorkspaceChangesResponse | null {
  if (!workspace) return null;
  ...
  next[params.sourceSection] = sourceItems.filter((item) => {
    ...
    const destination =
      typeof params.destinationSection === "function"
