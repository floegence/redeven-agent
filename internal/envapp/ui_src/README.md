...
- That Redeven-owned workbench input-routing layer now relies on the released floe-webapp live canvas-measurement and widget-local activation contracts for keyboard navigation, shell-driven focus/ensure handoffs, and first-click virtual-input activation, so arrow-key navigation and cross-surface activation keep centering correctly without Redeven-local contract forks.
...
- Workbench-local dialogs now follow the released `data-floe-local-interaction-surface="true"` contract from floe-webapp: dialog overlays keep widget-boundary semantics, pointer/wheel/context-menu routing yields to that local surface, outside widgets do not auto-close the dialog, and `Escape` still resolves from the focused boundary instead of broadcasting across the canvas.
...
- Redeven workbench widget identity is also `widget.id`-stable: click-to-front, focus routing, move, and resize only update the surface snapshot, so ordinary mounted business bodies such as Monitoring and Files keep their in-memory session state instead of remounting. Expensive live surfaces use an explicit lifecycle overlay instead: Terminal renders a real Ghostty-backed panel only while the widget is `hot`, and inactive terminal widgets render a lightweight paused preview while keeping semantic session state intact.
...
- Terminal workbench scaling is lifecycle-gated. A workbench may contain many terminal widgets, but only the selected hot widget owns live renderer resources, RPC subscriptions, focus-triggered resize, and history replay. Warm/cold terminal widgets keep their runtime-shared session IDs and local active-tab choice, then hydrate only when the user resumes that widget.
