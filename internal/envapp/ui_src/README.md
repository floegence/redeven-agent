...
- Heavy workbench business widgets now opt into floe-webapp's shared `renderMode: 'projected_surface'` contract: Files, Terminal, Preview, Codespaces, Flower, and Codex move their rich DOM bodies onto the live overlay surface while keeping the same persisted world-space layout model, and lighter widgets such as Monitoring/Ports stay on the default `canvas_scaled` path.
