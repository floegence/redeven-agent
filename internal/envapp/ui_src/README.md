...
- Workbench file-browser and file-preview widgets now opt into the released floe-webapp `renderMode: 'projected_surface'` contract. They keep the same persisted world geometry and workbench focus/fronting behavior, but their business DOM mounts in the live viewport overlay layer instead of inside the canvas scale transform ancestor, which keeps Monaco and other rich preview surfaces stable under zoom.
