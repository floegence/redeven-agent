...
- `Activity` mode keeps the activity bar and shell sidebar model; `Deck` mode reuses the shared `DeckTopBar` chrome and exposes the singleton surface catalog as composable grid widgets; `Workbench` mode keeps the released floe-webapp workbench state/style contract but wraps it in a Redeven-owned input-routing surface so widget-local scroll, DOM focus, and shell hotkey ownership behave correctly for real Env App business widgets.
