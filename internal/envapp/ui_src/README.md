...
- Text preview Monaco boot now consumes floe-webapp's released profile-first runtime contract: read-only preview uses `runtimeOptions.profile = 'preview_basic'` so preview surfaces keep syntax highlighting with the lean runtime, while edit mode remounts onto `editor_full` before enabling Monaco's richer editable service graph.
