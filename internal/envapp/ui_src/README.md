...
- Workbench wheel ownership is also selection-first: generic `data-floe-canvas-interactive="true"` regions stay eligible for cursor-centered canvas zoom, `data-floe-local-interaction-surface="true"` descendants still keep local overlay/dialog ownership, and explicit `data-floe-canvas-wheel-interactive="true"` business surfaces only keep wheel local when they belong to the currently selected widget.
