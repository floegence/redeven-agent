...
- Env details shell navigation is now explicitly split into container mode vs business surface state: `EnvViewMode` (`tab`, `deck`, `infinite_map`) selects the shell container, while `EnvSurfaceId` (`terminal`, `monitor`, `files`, `codespaces`, `ports`, `ai`, `codex`) identifies the business surface that handoffs and persistence target.
- `Deck` is no longer an activity-bar tab id. Desktop uses a top-bar mode switcher to move between `Tab`, `Deck`, and the current `Infinite map` placeholder, while mobile always resolves to `tab` without overwriting the stored desktop mode preference.
- `Tab` mode keeps the activity bar; `Deck` mode hides it and exposes the full singleton surface catalog, including Flower and Codex, as composable deck widgets; `Infinite map` is wired as a stable persisted mode contract now but still renders a placeholder surface.
- Cross-surface requests should route through the shell-owned `openSurface(...)` / mode-routing contract in `EnvAppShell` instead of assuming page-local `goTab(...)` semantics. In `deck` mode the shell focuses or inserts the target widget, and opening Runtime Settings intentionally falls back through `tab` mode because settings remains a page-owned workbench.
- Shared runtime providers stay above the mode split. Terminal runtime sessions remain coordinator-owned, so moving between `tab` and `deck` remounts the visible surface without closing PTY sessions; Flower and Codex keep their controller/session ownership while swapping between shell-sidebar (`tab`) and inline rail (`deck`) containers.
